...
__all__ = ["Compose", "ToTensor", "PILToTensor" , "Normalize", "Resize", "Scale",
           "CenterCrop", "Pad", "Lambda", "RandomApply", "RandomChoice", "RandomOrder",
           "RandomHorizontalFlip", "RandomVerticalFlip",
           "LinearTransformation", "RandomAffine", "Grayscale", "RandomGrayscale",
...
class LinearTransformation():
    ...
    offline.
    This transform does not support PIL Image.
    Given transformation_matrix and mean_vector, will flatten the paddle.*Tensor and
    ...
    Applications:
        ...
        Then compute the data covariance matrix [D x D] with paddle.mm(X.t(), X),
    ...
    def forward(self, tensor: Tensor) -> Tensor:
        ...
        transformed_tensor = paddle.mm(flat_tensor, self.transformation_matrix)
...
class RandomAffine():
    ...
    def get_params(
        ...
        """
        angle = float(paddle.empty(1).uniform_(float(degrees[0]), float(degrees[1])).item())
        if translate is not None:
            max_dx = float(translate[0] * img_size[0])
            max_dy = float(translate[1] * img_size[1])
            tx = int(round(paddle.empty(1).uniform_(-max_dx, max_dx).item()))
            ty = int(round(paddle.empty(1).uniform_(-max_dy, max_dy).item()))
            translations = (tx, ty)
        else:
            translations = (0, 0)

        if scale_ranges is not None:
            scale = float(paddle.empty(1).uniform_(scale_ranges[0], scale_ranges[1]).item())
        else:
            scale = 1.0

        shear_x = shear_y = 0.0
        if shears is not None:
            shear_x = float(paddle.empty(1).uniform_(shears[0], shears[1]).item())
            if len(shears) == 4:
                shear_y = float(paddle.empty(1).uniform_(shears[2], shears[3]).item())
...
class RandomErasing():
    ...
    def get_params(
        ...
        log_ratio = paddle.log(paddle.tensor(ratio))
        for _ in range(10):
            erase_area = area * paddle.empty(1).uniform_(scale[0], scale[1]).item()
            aspect_ratio = paddle.exp(
                paddle.empty(1).uniform_(log_ratio[0], log_ratio[1])
            ).item()
            ...
            if value is None:
                v = paddle.empty([img_c, h, w], dtype=paddle.float32).normal_()
            else:
                v = paddle.tensor(value)[:, None, None]

            i = paddle.randint(0, img_h - h + 1, size=(1, )).item()
            j = paddle.randint(0, img_w - w + 1, size=(1, )).item()
            return i, j, h, w, v
...
class GaussianBlur():
    ...
    def get_params(sigma_min: float, sigma_max: float) -> float:
        ...
        """
        return paddle.empty(1).uniform_(sigma_min, sigma_max).item()
...
class RandomPosterize():
    ...
    number of bits for each color channel. If the image is torch Tensor, it should be of type paddle.uint8,
