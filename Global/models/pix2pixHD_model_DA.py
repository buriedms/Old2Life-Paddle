...
class Pix2PixHDModel(BaseModel):
    ...
    def init_loss_filter(self, use_gan_feat_loss, use_vgg_loss):
        ...
        def loss_filter(g_gan, g_gan_feat, g_vgg, g_kl, d_real, d_fake, g_featd, featd_real, featd_fake):
            return [l for (l, f) in
                    zip((g_gan, g_gan_feat, g_vgg, g_kl, d_real, d_fake, g_featd, featd_real, featd_fake), flags) if f]
    ...
    def initialize(self, opt):
        ...
        if not self.isTrain or opt.continue_train or opt.load_pretrain:
            ...
            if self.isTrain:
                ...
                self.load_network(self.feat_D, 'featD', opt.which_epoch, pretrained_path)
        ...
        if self.isTrain:
            ...
            self.loss_names = self.loss_filter('G_GAN', 'G_GAN_Feat', 'G_VGG', 'G_KL', 'D_real', 'D_fake', 'G_featD',
                                               'featD_real', 'featD_fake')
    ...
    def encode_input(self, label_map, inst_map=None, real_image=None, feat_map=None, infer=False):
        if self.opt.label_nc == 0:
            ...
            input_label = paddle.ones(oneHot_size, dtype=paddle.float32)
        ...
        if self.use_features:
            ...
            if self.opt.load_features:
                feat_map = paddle.to_tensor(feat_map)
    ...
    def forward(self, label, inst, image, feat, infer=False):
        ...
        real_old_feat = []
        syn_feat = []
        inst_numpy=inst.numpy()
        for index,x in enumerate(inst_numpy):
            if x == 1:
        ...
        syn_feat = syn_feat[:L]
        real_old_feat = paddle.concat(real_old_feat, axis=0)
        syn_feat = paddle.concat(syn_feat, axis=0)
    ...
    def update_learning_rate(self):
        ...
        self.optimizer_featD.set_lr(lr)
